...
def test_sanitize_columns_replaces_spaces_dots_dashes():
    ...
    print("✅ sanitize_columns() : OK")

# --- try_parse_dates() ---
def test_try_parse_dates_parses_and_sets_nan():
    df = pd.DataFrame({"Visit Date": ["2024-01-01", "not a date"]})
    df = loader.sanitize_columns(df)  # -> Visit_Date
    out = loader.try_parse_dates(df)
    assert str(out["Visit_Date"].iloc[0]).startswith("2024-01-01")
    assert pd.isna(out["Visit_Date"].iloc[1])
    print("✅ try_parse_dates() : OK")

# --- main() : cas nominal (insertion + dédoublonnage) ---
def test_main_happy_path_monkeypatched(monkeypatch):
    ...
    assert coll.count_documents({}) == 2  # doublon supprimé
    print("✅ main() (cas nominal) : OK")

# --- main() : CSV manquant -> exit(1) ---
def test_main_exits_when_csv_missing(monkeypatch):
    ...
    monkeypatch.setattr(pd, "read_csv", lambda path: None)

    with pytest.raises(SystemExit) as e:
        loader.main()
    assert e.value.code == 1
    print("✅ main() (CSV manquant) : OK")
